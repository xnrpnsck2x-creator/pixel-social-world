...
static func add_mail_row(parent: VBoxContainer, message: Dictionary, app: Node, action: Callable) -> void:
	...
	add_row(parent, "icon.mail", title, str(message.get("body", "")), str(app.call("t_key", "messages.mail.read")), action)

static func add_private_row(parent: VBoxContainer, message: Dictionary, app: Node) -> void:
	var sender := str(message.get("sender_id", ""))
	var title := str(app.call("format_key", "messages.private.row_title_format", {"sender": sender}))
	add_row(parent, "icon.chat", title, str(message.get("body", "")))

static func add_conversation_row(parent: VBoxContainer, conversation: Dictionary, app: Node, action: Callable) -> void:
...
static func add_empty_row(parent: VBoxContainer, icon_id: String, title: String) -> void:
	add_row(parent, icon_id, title, "")
