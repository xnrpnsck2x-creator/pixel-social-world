...
@export var pixel_scale := 0.095
@export var rise_distance := 1.5
...
func play(emote_id: String) -> void:
	...
	_tween.parallel().tween_property(_sprite, "scale", Vector2.ONE * pixel_scale, 0.16).from(Vector2.ONE * 0.06)
	...
	_tween.parallel().tween_property(_sprite, "position", Vector2(0, -rise_distance - 5.0), 0.22)
	_tween.tween_callback(_hide)

func _reset_sprite() -> void:
	_sprite.position = Vector2.ZERO
	_sprite.scale = Vector2.ONE * 0.06
