...
SOURCE_PATH = ROOT / "assets" / "sprites" / "generated" / "npc_professions_direction_v1_alpha.png"
OUT_DIR = ROOT / "assets" / "sprites" / "sliced" / "npc_professions_direction_v1"
CONFIG_PATH = ROOT / "configs" / "npc_professions.json"
ART_ASSETS_PATH = ROOT / "configs" / "art_assets.json"
PREVIEW_PATH = ROOT / ".tools" / "npc-profession-direction-v1" / "npc_professions_direction_v1_preview.png"
ALPHA_THRESHOLD = 18
PADDING = 6
SPRITE_SCALE = 0.14
...
ROLES: tuple[RoleSpec, ...] = (
    RoleSpec("merchant_v1", "merchant", "main_city_trade_and_market_maps"),
    RoleSpec("mail_courier_v1", "mail_courier", "main_city_mail_and_messaging"),
    RoleSpec("game_host_v1", "game_host", "main_city_minigame_and_event_host"),
    RoleSpec("home_keeper_v1", "home_keeper", "housing_and_cozy_service_maps"),
    RoleSpec("academy_registrar_v1", "academy_registrar", "academy_creator_and_review_maps"),
)

# The generated Image 2 sheet visually returned rows as down, left, up, right.
...
def main() -> None:
    ...
    print(f"Wrote {len(generated_records)} NPC profession direction slices")
...
def crop_sprite_cell(image: Image.Image, col: int, row: int) -> Image.Image:
    ...
    if xs.size == 0 or ys.size == 0:
        raise RuntimeError(f"Empty NPC sprite cell col={col} row={row}")
...
def update_npc_professions(generated_records: list[dict]) -> None:
    ...
    payload["source_sheet"] = to_resource_path(SOURCE_PATH)
...
def update_art_assets(generated_records: list[dict]) -> None:
    ...
    remove_ids = {
        "sprite.sheet.npc_professions.direction_v1",
        "sprite.slices.npc_professions.direction_v1",
    }
    remove_ids.update(
    ...
    assets.append({
        "id": "sprite.sheet.npc_professions.direction_v1",
        "type": "generated_sheet",
        "path": to_resource_path(SOURCE_PATH),
        "source_path": to_resource_path(ROOT / "assets" / "sprites" / "generated" / "npc_professions_direction_v1_source.png"),
        "usage": "formal_directional_npc_profession_sheet",
    })
    assets.append({
        "id": "sprite.slices.npc_professions.direction_v1",
        "type": "generated_slices",
        "path": to_resource_path(OUT_DIR),
        "source_path": to_resource_path(SOURCE_PATH),
        "usage": "main_city_npc_profession_direction_frames",
    })
    for record in generated_records:
        assets.append({
