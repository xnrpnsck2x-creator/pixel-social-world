...
| iOS / Android export | 87% device-in-progress | Native presets, Image 2 store branding assets, iOS toolchain detection, Android SDK/JDK/build-tools/CMake/NDK setup, accepted Android licenses, native preset parse checks, locally signed/pruned Android debug APK export, APK asset-budget guard, streamed adb install/launch, Android online auth/presence/WS, landscape login/main-city/housing/fishing/tap-to-move/chat/trade keyboard smoke, Android runtime perf v1.1, Android package budget v1, Android UI interaction polish v1, and Android device interaction QA v2 are verified. | Release signing, iOS true-device pass, store auth providers, push/notification decisions, and deeper native render profiling. |
...
| Android client runtime | 3.4 | 3.4 | 3.2 | True-device debug build caps mobile rendering at 30 FPS, idles tap/NPC ambience processing, uses clearer icon-only HUD action buttons, has a narrowed main-city trade hotspot for tap-to-move safety, refreshes presence immediately after native login, and rejects low-wallet Home catalog placement before optimistic local placement. Post-change open-world samples are roughly 36% process CPU with about 253 MB PSS / 224 MB RSS. APK size is now 147.3 MB after pruning generated source caches, Android-excluded launch splash payloads, and retired map candidates. |
