...
| Presence + room members | 76% | Heartbeat, room member list, remote avatars, profile card entry, stale state visible. | True disconnect edge cases and room-shard load tests. |
| WebSocket movement + emotes | 75% | Authenticated WS join, room fanout, movement interpolation, RO-style overhead emotes, repeatable 24/50/100-client load smoke. | Interest management and slow-client drop policy tuning. |
...
| WebSocket city hub | 3.4 | 3.2 | 3.8 | One connection per client is fine; same-room broadcast cost grows with room population, now tracked globally and per room. |
| Movement sync | 3.2 | 3.0 | 4.0 | Current 0.12s send interval is playable, and 24/50/100-client local fanout smoke passes; dense rooms still need interest culling or lower update rates. |
...
- WebSocket write backpressure from slow clients.
...
1. Add a deterministic slow-client test and choose the first drop/backoff behavior.
2. Add room-size policy knobs for main city, homes, and minigame rooms.
3. Define retention rules for private messages, mail, reports, and ledgers before public alpha.
4. Run a real-device H5/mobile keyboard and FPS pass after the next UI slice.
5. Start a Redis-mode WS fanout smoke once local Redis is part of the regular dev loop.
