...
| iOS / Android export | 95% device-verified | Native presets, Image 2 store branding assets, iOS toolchain detection, iOS release signing contract preflight, Android SDK/JDK/build-tools/CMake/NDK setup, accepted Android licenses, native preset parse checks, locally signed/pruned Android debug APK export, APK asset-budget guard, Android release signing contract preflight, streamed adb install/launch, Android online auth/presence/WS, landscape login/main-city/housing/fishing/tap-to-move/chat/trade keyboard smoke, Android runtime perf v1.1, Android package budget v1, Android UI interaction polish v1, Android device interaction QA v2, Android stability probe v1, native render throttle v1, Android runtime budget gate v1, 600-second Android soak, and full Android device regression are verified. | Actual iOS signing/profile handoff, iOS true-device pass, Android release keystore handoff, store auth providers, push/notification decisions, and production-scale soak/load profiling. |
