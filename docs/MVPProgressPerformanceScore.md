...
| Private messages + mail | 80% | Durable private conversations, unread summaries, read markers, report endpoint, mailbox base, block enforcement, retention windows, cleanup dry-run command, and daily systemd cleanup timer. | Notification UX, inbox pagination polish, and production retention dry-run review. |
...
| Economy | 83% | Wallet, ledger, trusted spends, trusted fishing reward grants, idempotent reward claims, creator revenue share policy, owner-only idempotent creator settlement endpoint, and daily reward soft cap. | Anti-inflation dashboards and per-game payout analytics. |
...
| LiveOps / moderation | 77% | Admin roles, chat reports, profile reports, moderation audit, debug ops, per-room realtime WS counters, retention/economy policy visibility, cleanup-plan metadata, cleanup runner, timer packaging, CSV exports. | Real operator auth, alerting, dashboards beyond local H5 tooling. |
...
Overall MVP implementation forecast: 84-88%.
Public alpha readiness forecast: 67-72%.
...
| Private messages / mail | 4.0 | 3.9 | 4.1 | Durable rows are manageable; retention windows now have a dry-run/execute cleanup command plus a daily systemd timer, while pagination remains the next storage control. |
...
- PostgreSQL growth from private messages, mail, ledgers, reports, and creator audit history; daily retention cleanup now controls old rows, but production dry-runs should be reviewed before first execution.
...
3. Add creator payout analytics dashboards and cap-hit reporting.
