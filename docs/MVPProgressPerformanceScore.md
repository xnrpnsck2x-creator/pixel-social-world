...
| LiveOps / moderation | 100% pre-device | Admin roles, chat reports, profile reports, moderation audit, unified high-risk admin action audit, trade history audit with CSV handoff, debug ops, per-room realtime WS counters, retention/economy policy visibility, creator payout/cap-hit counters, creator payout drilldowns, cleanup-plan metadata, cleanup runner, timer packaging, CSV exports, initial alpha alert thresholds, `/debug/ops.alerts` wiring, trade risk alerting, lightweight alert forwarding, Ubuntu systemd alert probe packaging, production monitoring handoff gate, Production data backup handoff gate, LiveOps smoke, chat reports smoke, chat moderation audit smoke, reviewer backend E2E, and H5 small-screen semantic coverage. | Real operator identity provider, external audit sink, external monitoring receiver, release-host strict monitoring env, and release-host backup/restore drill are production-environment validation items. |
...
| iOS / Android export | 98% device-verified | Native presets, Image 2 store branding assets, iOS toolchain detection, iOS release signing contract preflight, Android SDK/JDK/build-tools/CMake/NDK setup, accepted Android licenses, native preset parse checks, locally signed/pruned Android debug APK export, Android Play AAB preset with target SDK 35, APK/AAB release readiness contracts, APK asset-budget guard, native release handoff gate, App Store / Google Play publish handoff gate, streamed adb install/launch, Android online auth/presence/WS, landscape login/main-city/housing/fishing/tap-to-move/chat/trade keyboard smoke, Android runtime perf v1.1, Android package budget v1, Android UI interaction polish v1, Android device interaction QA v2, Android stability probe v1, native render throttle v1, Android runtime budget gate v1, 600-second Android soak, and full Android device regression are verified. | Actual iOS strict signing/profile pass on release machine, iOS true-device pass, Android release keystore strict pass, App Store Connect / Play Console strict metadata evidence, store auth providers, push/notification decisions, and production-scale soak/load profiling. |
...
Public Alpha release readiness: locally store-publish gated; awaiting iOS true-device, strict store-auth, strict signing, App Store Connect / Play Console evidence, and production-monitoring verification.
...
| Android client runtime | 3.8 | 4.1 | 3.5 | True-device debug build now caps mobile rendering at 24 FPS, runs physics at 30 ticks per second, enables low-processor sleep, idles avatar/nameplate/remote interpolation processing, and pauses closed realtime polling. The 240-second route probe on `c7e94055` improved CPU to 23.5% avg / 32% max with stable memory; the 600-second route soak held at 20.8% avg / 32.6% max CPU, 314.9 MB avg PSS, 341.5 MB max PSS, -30.2 MB PSS growth, and 20.6 MB max swap PSS. The Android runtime budget gate now enforces CPU, PSS, growth, swap, and sample coverage budgets. APK size remains 147.3 MB after pruning generated source caches, Android-excluded launch splash payloads, and retired map candidates. |
...
- PostgreSQL growth from private messages, mail, ledgers, reports, and creator audit history; paged reads, daily retention cleanup, and the production data backup handoff now control the first growth/recovery risks, but production dry-runs and restore drills should be reviewed before first execution.
...
- Android release signing contract preflight now verifies the APK/AAB format contract, Java/build-tools, `zipalign`, `apksigner`, `keytool`, and no committed signing values before real release keystore env is introduced.
- Native release handoff v1 is now documented in `docs/NativeReleaseHandoffRunbook.md` and checked by `scripts/check_native_release_handoff.sh`, including strict-mode fail-closed behavior when Android/iOS signing env is absent.
- App Store / Google Play publish handoff v1 is now documented in
  `docs/StorePublishHandoff.md` and checked by
  `scripts/check_store_publish_handoff.sh`. It verifies Apple/Google official
  requirement source links, Android Play AAB readiness with target SDK 35, store
  branding assets, no committed store secrets, and strict-mode fail-closed
  behavior when App Store Connect / Play Console evidence env is absent.
- Android runtime budget gate v1 validates the 240-second render-throttle report and the 600-second soak report. The project category v2 gate now executes this as `android_runtime_budget_reports_pass`. The 600-second soak evidence is under `.tools/android-stability-soak-v1/` and passed with 36 samples, 20.8% average CPU, 341.5 MB peak PSS, and -30.2 MB PSS growth.
...
- Production monitoring handoff v1 is now documented in `docs/ProductionMonitoringHandoff.md` and checked by `scripts/check_production_monitoring_handoff.sh`, including strict-mode fail-closed behavior when monitoring env is absent.
- Production data backup handoff v1 is now documented in `docs/ProductionDataBackupHandoff.md` and checked by `scripts/check_production_data_backup_handoff.sh`, including strict-mode fail-closed behavior when backup env is absent.
...
3. Run `scripts/check_store_publish_handoff.sh` in strict mode with external App Store Connect / Google Play Console evidence env.
4. Run the Android release readiness script in AAB strict mode with the external keystore env on the release machine.
5. Run the iOS release readiness script in strict mode with external signing/profile env, then run the iOS true-device pass.
6. Run release-mode Android and iOS device soaks once store signing and physical iOS access are available.
7. Run `scripts/check_production_monitoring_handoff.sh` in strict mode on the production host, then wire the LiveOps alert probe output to the final external monitoring receiver.
8. Run `scripts/check_production_data_backup_handoff.sh` in strict mode on the production host, then complete one restore drill into disposable PostgreSQL and creator package directories.
9. Run `PSW_POSTGRES_TEST_DSN=... go test ./internal/trade -run 'TestPostgresTradePurchasePersistsLedgerInventoryAndReplayGuards|TestPostgresConcurrentPurchaseAllowsOneBuyer' -v` against the Ubuntu/Postgres target before public alpha.
