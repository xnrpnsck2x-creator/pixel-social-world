...
| iOS / Android export | 96% device-verified | Native presets, Image 2 store branding assets, iOS toolchain detection, iOS release signing contract preflight, Android SDK/JDK/build-tools/CMake/NDK setup, accepted Android licenses, native preset parse checks, locally signed/pruned Android debug APK export, APK asset-budget guard, Android release signing contract preflight, native release handoff gate, streamed adb install/launch, Android online auth/presence/WS, landscape login/main-city/housing/fishing/tap-to-move/chat/trade keyboard smoke, Android runtime perf v1.1, Android package budget v1, Android UI interaction polish v1, Android device interaction QA v2, Android stability probe v1, native render throttle v1, Android runtime budget gate v1, 600-second Android soak, and full Android device regression are verified. | Actual iOS strict signing/profile pass on release machine, iOS true-device pass, Android release keystore strict pass, store auth providers, push/notification decisions, and production-scale soak/load profiling. |
...
6. Run `scripts/check_production_monitoring_handoff.sh` in strict mode on the production host, then wire the LiveOps alert probe output to the final external monitoring receiver.
7. Run `scripts/check_production_data_backup_handoff.sh` in strict mode on the production host, then complete one restore drill into disposable PostgreSQL and creator package directories.
8. Run `PSW_POSTGRES_TEST_DSN=... go test ./internal/trade -run 'TestPostgresTradePurchasePersistsLedgerInventoryAndReplayGuards|TestPostgresConcurrentPurchaseAllowsOneBuyer' -v` against the Ubuntu/Postgres target before public alpha.
