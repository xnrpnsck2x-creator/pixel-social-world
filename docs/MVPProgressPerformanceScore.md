...
| iOS / Android export | 93% device-verified | Native presets, Image 2 store branding assets, iOS toolchain detection, Android SDK/JDK/build-tools/CMake/NDK setup, accepted Android licenses, native preset parse checks, locally signed/pruned Android debug APK export, APK asset-budget guard, streamed adb install/launch, Android online auth/presence/WS, landscape login/main-city/housing/fishing/tap-to-move/chat/trade keyboard smoke, Android runtime perf v1.1, Android package budget v1, Android UI interaction polish v1, Android device interaction QA v2, Android stability probe v1, native render throttle v1, Android runtime budget gate v1, 600-second Android soak, and full Android device regression are verified. | Release signing, iOS true-device pass, store auth providers, push/notification decisions, and production-scale soak/load profiling. |
...
3. Run release-mode Android and iOS device soaks once store signing and physical iOS access are available.
4. Wire the systemd LiveOps alert probe output to the final external monitoring receiver.
5. Run `PSW_POSTGRES_TEST_DSN=... go test ./internal/trade -run 'TestPostgresTradePurchasePersistsLedgerInventoryAndReplayGuards|TestPostgresConcurrentPurchaseAllowsOneBuyer' -v` against the Ubuntu/Postgres target before public alpha.
