...
static func verify_creator_package_publish(root: Node, failures: Array) -> void:
	...
	if int(creator_catalog.get("status", 0)) != 200:
		...
		for item in catalog_items:
			if typeof(item) != TYPE_DICTIONARY or str((item as Dictionary).get("game_id", "")) != "creator_e2e_package":
				continue
			for private_field in ["install_uri", "manifest_uri", "source_storage_key", "install_key"]:
				if (item as Dictionary).has(private_field):
					failures.append("Creator catalog exposed private field: %s." % private_field)
	var published_runtime := await raw_json(
		root,
		HTTPClient.METHOD_GET,
		"/minigames/creator_e2e_package/runtime",
		{},
		""
	)
	var runtime_body := published_runtime.get("body", {}) as Dictionary
	var runtime_definition := runtime_body.get("definition", {}) as Dictionary
	if int(published_runtime.get("status", 0)) != 200 or str(runtime_definition.get("game_id", "")) != "creator_e2e_package":
		failures.append("Published creator runtime was not player-readable.")
	for private_field in ["install_uri", "manifest_uri", "source_storage_key", "install_key"]:
		if runtime_body.has(private_field):
			failures.append("Creator runtime exposed private field: %s." % private_field)
	...
	var unpublished_runtime := await raw_json(
		root,
		HTTPClient.METHOD_GET,
		"/minigames/creator_e2e_package/runtime",
		{},
	...
	if int(unpublished_runtime.get("status", 0)) != 404:
		failures.append("Unpublished creator runtime remained player-readable.")
